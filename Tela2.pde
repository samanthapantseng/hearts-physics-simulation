class Tela2 {
  ...
  public Tela2(ParticleSystem mundoVirtual, int cantidad) {
    ...
    for (int i = 0; i < cantidadDeParticulasPorLado; i++) {
      for (int j = 0; j < cantidadDeParticulasPorLado; j++) {
        arrayDeParticulas[i][j] = mundoVirtual.makeParticle(0.25, (width*1/5) + j * pasoEnX-(width/19), (i * pasoEnY)-height/17, 0.0); // ( float mass, float x, float y, float z )
